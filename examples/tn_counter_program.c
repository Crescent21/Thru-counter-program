...
static void handle_decrement_counter(
    ...
    if (counter_account->counter_value == 0) {
        tsdk_return(TN_COUNTER_ERR_COUNTER_UNDERFLOW);
    }
...
static void handle_set_counter(
    uchar const *instruction_data,
    ulong instruction_data_sz TSDK_PARAM_UNUSED
) {
    tn_counter_set_args_t const *args =
        (tn_counter_set_args_t const *)instruction_data;

    ushort account_idx = args->account_index;
    ulong new_value = args->new_value;
    ...
    /* Set counter to the requested value */
    tn_counter_account_t *counter_account =
        (tn_counter_account_t *)account_data;
    counter_account->counter_value = new_value;
...
static void handle_increment_counter(uchar const *instruction_data, ulong instruction_data_sz TSDK_PARAM_UNUSED) {
    ...
    /* Increment the counter (prevent overflow) */
    tn_counter_account_t* counter_account = (tn_counter_account_t*)account_data;

    if (counter_account->counter_value == ULONG_MAX) {
        tsdk_return(TN_COUNTER_ERR_COUNTER_OVERFLOW);
    }
...
case TN_COUNTER_INSTRUCTION_RESET:
        ...
        break;
    case TN_COUNTER_INSTRUCTION_SET:
        if (instruction_data_sz != sizeof(tn_counter_set_args_t)) {
            tsdk_revert(TN_COUNTER_ERR_INVALID_INSTRUCTION_DATA_SIZE);
        }

        handle_set_counter(instruction_data, instruction_data_sz);
        break;
    default:
